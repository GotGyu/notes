...
- 由于容器内部运行的应用不同，内存使用模式也不尽相同，**对内存管理策略有不同的定制化需求**（如：mysql受益于透明大页访存，可单独设置启用透明大页；而redis则推荐关闭此功能），然而memory cgroup**作用于物理内存一侧**为应用提供限制隔离功能，无法隔离下层物理内存组织模式的策略，也缺少对管理上层虚拟内存分配的虚拟内存超配策略隔离的支持，所以它存在很大局限性
...
1. **定义**
   ...
   - Linux 内核默认使用 **4KB 小页（normal page）** 进行内存管理，但现代 CPU 支持 **大页（huge page）**（如 2MB、1GB），可减少 TLB 开销，提高内存访问效率
