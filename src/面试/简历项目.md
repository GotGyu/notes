...
- 若**使用Protocol Buffers（protobuf）作为接口定义语言**，定义严格的接口和数据结构，这有助于避免数据格式不一致的问题。gRPC 使用带特殊 gRPC 插件的 `protoc` 从 `.proto` 文件生成代码，从而可以获得生成的 gRPC 客户端和服务器代码，以及用于填充、序列化和检索消息类型的常规 pb 代码。比如下图中的2-11步全都封装起来，对用户透明
  ...
  ![image-20250313213022181](../utils/grpc.png)
...
1. 过程宏：用于自动注册钩子函数，自动化重复模式，减少样板代码
...
![1742046300013](../utils/quantstep.png)
...
- 采用inline hook。在函数入口处插入一个 `jmp` 指令，并直接指向自定义函数的入口地址。当内核调用该函数时，会首先跳转到自定义函数的入口处执行自定义的代码，完成操作后再跳转回原函数继续执行。由于 Inline Hook 技术采用的是基于 `jmp` 指令的重定向技术，不需要保存和恢复寄存器状态，也不需要进行额外的栈操作，因此可以极大地减少重定向操作的开销
  ...
  ![image-20250325150510413](../utils/vkernel6.png)
...
5. **Linux内核中的实现**
   ...
   - **软件支持**
     ...
     - 目前 Linux 透明大页机制**只作用于匿名页**，并通过**两种方式**为进程分配使用大页，总结如图：
       ...
       - 方式一：进程访问虚拟地址，触发缺页中断时，如果该虚拟地址为一个连续的大页面，OS立刻分配对应的连续物理大页，如果分配失败，OS会执行内存碎片化整理，通过页面迁移创建一个空闲的物理大页；这个过程会阻塞缺页中断过程，增大缺页延迟，在内存碎片化严重时，页面迁移仍然可能分配不出连续的物理大页。此外，在大页分配结束后，会被立刻初始化，**这个过程增大了第一次缺页中断延迟，但可以减少后续内存访问的平均延迟**
     
       - 方式二：Linux 内核使用一个内核线程 `khugepaged`( Linux 中自动合并小页面为透明大页的后台线程)，异步扫描系统上的页表，当发现一个连续的 2MB 匿名虚拟内存区域并且包含至少一个 4KB 映射后，该线程会尝试为该虚拟内存区域分配并映射一个物理大页，如果分配失败则会触发内存压缩机制，在分配出物理大页时，该线程会把包含的 4KB 页迁移到分配的物理大页上。在 `khugepaged` 线程作用时会阻塞进程对该虚拟地址区域的访问
...
2. **一句话总结**
   ...
   部分容器应用依赖内存超配机制，而部分容器应用在物理内存紧张时需要禁用内存超配机制增强稳定性
...
6. **为什么Redis应用要将 `overcommit_memory` 设置为1**
...
7. **为什么Postgresql应用要将 `overcommit_memory` 设置为0**
...
![vkernel架构图](../utils/vkernel7.png)

1. **Rust中间层组件**：

   - 用于扩展 Rust 内核模块的基础设施，为vkernel提供基础支持，需要扩展  `bindings` 与 `kernel` Rust包
...
2. **重定向组件**：
   ...
   ![image-20250325154403669](../utils/vkernel8.png)
...
3. **容器内存管理策略自适应问题**
   ...
   - **解决方案**
     ...
     - 设计用户态策略定制组件，动态监测vkernel提供的容器应用内存指标，为容器动态指定特定的管理参数
       ...
       ![image-20250326102832863](../utils/vkernel9.png)
...
- 基本内核模块结构
  ...
  module! {
      ...
      name: b"vkernel",
      author: b"Your Name",
      description: b"vKernel - Secure Container Runtime Module",
      license: b"GPL",
  }
  
  struct VKernelModule;
  
  impl kernel::Module for VKernelModule {
      fn init(_name: &'static CStr, _module: &'static ThisModule) -> Result<Self> {
          pr_info!("vKernel module loaded\n");
  ...
  impl Drop for VKernelModule {
      fn drop(&mut self) {
          pr_info!("vKernel module unloaded\n");
      }
  }
  ```
...
- 内核数据结构交互
  ...
  // 获取当前进程信息
  fn get_current_process_info() -> Result<()> {
      let current = Task::current().unwrap();
      
      pr_info!("Current process: pid={}, comm={}\n", 
          current.pid(), 
          current.comm());
  ...
  fn list_processes() -> Result<()> {
      pr_info!("Listing processes:\n");
      
      let mut task = unsafe { Task::from_raw(bindings::init_task) };
      let start = task.clone();
      
      loop {
          pr_info!("PID: {}, Name: {}\n", task.pid(), task.comm());
          
          unsafe {
              task = Task::from_raw((*task.as_ptr()).tasks.next);
          }
          
          if task.as_ptr() == start.as_ptr() {
              break;
          }
      }
...
- 内核功能调用
  ...
  // 调用内核打印功能
  fn kernel_print(message: &str) {
      unsafe {
          printk(
              KERN_INFO as *const i8, 
              c_str!("%s\0").as_ptr(), 
              c_str!(message).as_ptr()
          );
      }
  }
  ...
  fn check_capabilities() -> bool {
      unsafe { capable(CAP_NET_ADMIN) != 0 }
  }
  ```
...
- 文件系统交互
  ...
  impl FileOperations for VKernelFile {
      kernel::declare_file_operations!(read, write);
      
      fn read(&self, _file: &File, data: &mut UserSlicePtrWriter, _offset: u64) -> Result<usize> {
          let msg = "Hello from vKernel!\n";
          data.write_slice(msg.as_bytes())?;
          Ok(msg.len())
      }
      
      fn write(&self, _file: &File, data: &mut UserSlicePtrReader, _offset: u64) -> Result<usize> {
          let mut buf = [0u8; 256];
          let len = data.read(&mut buf)?;
          pr_info!("Received from userspace: {}\n", core::str::from_utf8(&buf[..len]).unwrap_or("[invalid utf8]"));
          Ok(len)
      }
  }
  ...
  fn register_device() -> Result<()> {
      let dev = fs::Registration::new_pinned(c_str!("vkernel"), 0, fs::Mode::S_IFCHR)?;
      dev.register(&VKernelFile)?;
      Ok(())
  }
  ```
...
- 内核模块参数
  ...
  // 定义可配置参数
  static DEBUG_MODE: kernel::module_param::ModuleParam<bool> = 
      kernel::module_param::ModuleParam::new(true, kernel::module_param::Perm::S_IWUSR | kernel::module_param::Perm::S_IRUGO, c_str!("debug_mode"));
...
- 内核通知链
  ...
  struct KeyboardNotifier;
  
  impl kernel::NotifierBlock for KeyboardNotifier {
      fn notifier_call(&self, nb: *mut notifier_block, action: u64, data: *mut c_void) -> i32 {
          // 处理键盘事件
          log_debug("Keyboard event detected");
          NOTIFIER_OK
      }
  }
  
  // 注册通知
  fn register_notifications() -> Result<()> {
      let nb = KeyboardNotifier;
      unsafe {
          register_keyboard_notifier(nb.as_mut());
      }
      Ok(())
  }
  ```

1. **安全边界**：所有与内核C代码的交互都需要使用`unsafe`块
2. **错误处理**：使用Rust的`Result`类型处理可能失败的操作
3. **内存管理**：内核模块不能使用标准库的内存分配，要使用`kernel::alloc`
4. **并发控制**：使用`kernel::sync`中的同步原语(Mutex, SpinLock等)
5. **性能考虑**：避免在关键路径上使用过多的Rust抽象
...
- **虚拟化**（如 KVM、Xen）关注资源的**复用与隔离**，多个 Guest OS 共享物理机；
- **TEE**（如 SGX、SEV、TrustZone）关注**执行的可信性与保密性**，即使 Host OS 被攻破，保护区内的执行也不被影响；
- 虚拟化保护的是“谁可以用资源”，TEE 保护的是“资源如何被信任地使用”。
- 具有**协同关系**，在云场景中，二者常常结合使用：虚拟化提供**多租户隔离与资源调度**、TEE 提供**计算可信性与保密执行**

### 怎么看待 TEE 在现代系统中的作用？它解决了什么问题？

TEE（Trusted Execution Environment）解决的是运行时的计算可信问题。它通过硬件隔离的方式，在一个受保护的环境中运行代码和数据，防止被宿主 OS、Hypervisor 等高权限组件窥探或篡改。

随着云计算的发展，多租户环境越来越普遍，云服务提供商和用户之间的信任边界变得模糊，TEE 就成为保护敏感数据（如模型权重、用户隐私数据）和代码的关键技术。现代 TEE 还支撑了“零信任”架构，是未来可信云基础的核心。

### TEE 的指令集，比如 Intel SGX、AMD SEV，你了解过吗？能否说下区别？

我对 SGX 和 SEV 都有基本了解：

- **Intel SGX** 提供了 enclave 模型，支持 finer-grained 隔离，开发者可以在 enclave 中运行特定函数，保证代码和数据的保密性与完整性。它基于一组特殊的指令（如 `EENTER`, `EEXIT`, `EGETKEY` 等）来控制执行流。
- **AMD SEV/SEV-ES/SNP** 是基于 VM 隔离的方案，提供的是虚拟机级别的内存加密，适合虚拟化环境中对整台 VM 加密保护。SNP 增加了 memory integrity 和 guest attestation 的能力。

SGX 更细粒度但开发成本高，SEV 更适合云场景的大范围部署。

### Hypervisor 是如何与 TEE 协同工作的？

Hypervisor 是 VMM，负责虚拟化底层资源，但在支持 TEE 的场景下，它通常**不被信任**。

- 在 SEV 架构中，Hypervisor 启动 Guest VM 时需要与 AMD 芯片中的 PSP（Platform Security Processor）交互，PSP 为 VM 提供加密密钥，Hypervisor 本身无法访问加密后的 VM 内存。
- 在 Intel TDX 或 ARM CCA 中，也存在 Hypervisor-Untrusted 模式，TEE/Realm 中的代码执行可以完全避免 Hypervisor 干预。

在我看来，Hypervisor 与 TEE 的协作关系微妙，Hypervisor 提供资源隔离，但必须在 TEE 架构下让出部分控制权，通过“不可见”实现真正的信任边界。

### 如果让你设计一个 container runtime 来支持 TEE，你会考虑哪些关键点？

- **镜像可信性**：通过签名和度量机制，确保拉取的是可信镜像；
- **启动时远程证明**：容器启动时执行 attestation；
- **隔离机制**：使用 SEV、TDX 等机制保护内存隔离；
- **I/O 路径保护**：数据必须走加密隧道，host 不得窃取；
- **兼容性**：能与 Kubernetes/PaaS 协作，提供标准化接口。

此外，为提升调试和部署体验，还需要支持 host-transparent 模式下的日志采集与故障恢复。

### 如果需要设计一个 TEE 中运行的容器环境，你会如何设计其软件栈？

我会从以下几个方面设计软件栈：

1. **启动链路**：
   - 使用可信 bootloader（如 TrenchBoot）引导 LibOS 或微内核；
   - Remote attestation 确保整个运行时状态可信（quote + measurement）。
2. **容器运行时（mini-container runtime）**：
   - 使用轻量 LibOS 作为容器 runtime；
   - 镜像通过签名校验（类似 cosign）；
   - 加密层存储，解密只在 enclave 内进行。
3. **I/O 路径**：
   - 使用 Host I/O Proxy 方式，容器本身使用加密通信；
   - 所有外发数据都进行 TLS 封装或文件系统加密（如 fs-verity）。
4. **优先加固模块**：
   - 镜像拉取与验证；
   - 内存页面生命周期与密钥使用控制；
   - enclave 运行期 syscall 路径（减少 TCB）；
   - 加密、签名、密钥封装模块必须由可信代码实现。

### TPM 与 TEE 有什么区别？它们能否配合使用？

TPM（Trusted Platform Module）是离 CPU 较远的一个独立芯片，主要用于开机前的测量和存储密钥；TEE 则是运行时的可信计算环境。

两者可以配合使用，例如：

- TPM 存储 enclave 签名密钥；
- TPM 验证 Boot 流程是否可信；
- TPM + TEE 联合提供可信链条，从 BIOS -> Bootloader -> Kernel -> TEE。

这种组合能够让可信链条从 cold boot 延伸到 runtime，是实现 end-to-end 可信执行的核心。

## Rust

### Rust 在系统层开发的优势？为什么选择用 Rust 实现 LibOS/Hypervisor？

Rust 具备零成本抽象和强静态内存安全，能帮助我们避免很多传统 C 系统代码中的 UAF、缓冲区溢出等漏洞，非常适合高安全要求的场景，如 TEE。

在我们构建 LibOS 或者 Rust Hypervisor 时，Rust 的 ownership model 可以精确控制资源生命周期，同时配合 `#![no_std]` 可以构建超小体积、确定性行为的执行环境。

尤其在机密计算中，**代码可信度直接影响信任边界**，Rust 可以从语言层减少攻击面和安全审计成本，这是非常关键的。

### 你如何看待 Rust 的 `async/await` 模型在 TEE/LibOS 中的适用性？

Rust 的 async/await 模型非常适合构建轻量并发模型。在 LibOS 中，如果使用传统线程模型，TCB 和调度开销较大；而 async 是无栈协程，通过 `Future` 实现更精细的控制流。

但也有挑战，比如：

- 在 `no_std` 场景下 async runtime 支持有限；
- pinning 和生命周期管理复杂；
- 部分裸机环境下没有定时器、IO 等 poll source，需要定制 poller。

我认为这块的发展方向是**打造轻量 async runtime for TEE/LibOS**，实现异步 + 安全 + 高性能三者的平衡。

### 用 Rust 开发运行于 TEE 中的服务，如何设计内存访问？需注意哪些 ABI 与裸机接口？

- 使用 `#![no_std]` + 裸机入口点（如 `#[no_mangle] pub extern "C" fn start()`）；
- 避免标准分配器，使用裸分配接口（如 buddy allocator 或 TLSF）；
- 使用 `#[repr(C)]` 保证与外部 ABI 对齐；
- 注意栈对齐、参数顺序、调用约定（如 AArch64 下为 `a0-a7` 传参）；
- 若和主机交换数据，需使用安全共享 buffer（需设置 non-enclave region 并复制）；
- memory-mapped I/O 需使用 volatile + fence 避免指令乱序。

## Linux与OS

### 你对 Linux 内核中与安全相关的机制了解多少？比如 capability、LSM？

我阅读过部分 Linux 源码，也分析过一些与 capability 相关的漏洞。

- Linux Capabilities 将 root 权限划分为多个子权限，比如 `CAP_SYS_ADMIN`、`CAP_NET_RAW` 等；
- 容器通过 capability whitelist 控制权限；
- 在安全容器/TEE 中，这套机制需要精细配置，否则可能引入攻击面（如著名的 CVE-2022-0492）。

LSM（如 SELinux、AppArmor）进一步提供强策略控制，在 TEE 中我们也可以复用这些机制，甚至以更严格的方式重新定义策略模型。

### 你了解 LibOS 吗？它和传统 OS、Unikernel 有什么区别？

是的，我了解 LibOS（Library Operating System）设计理念：

- **LibOS** 是将传统 OS 的 syscall 接口封装成用户态库，运行在用户态应用中；
- 它与 **Unikernel** 相似，都追求“应用即操作系统”，但：
  - Unikernel 是将应用和内核功能静态链接，生成单一镜像，运行在裸机或 hypervisor 上；
  - LibOS 是“在用户态复现 OS 接口”，通常运行在 TEE 或容器里，强调兼容性和精简性。

**关键区别：**

| 项目          | LibOS              | Unikernel        | 传统 OS       |
| ------------- | ------------------ | ---------------- | ------------- |
| 运行方式      | 用户态、隔离运行   | 裸机或Hypervisor | OS内核+用户态 |
| 接口目标      | 兼容 POSIX/syscall | 通常非 POSIX     | 完整 syscall  |
| 安全性/可信性 | 高（可组合TEE）    | 中（TCB大）      | 低（TCB大）   |

LibOS 是设计在可信执行环境中运行的小型系统组件，适合机密容器等需求。

### 如何在 LibOS 上运行一个 Rust 服务并确保 key 不泄露？

我会做以下设计：

- **运行环境：** Rust 服务部署在 TEE 内（如 SGX enclave、SEV-SNP VM 或 TrustZone secure world）；
- **密钥保护：**
  - 启动时通过 Remote Attestation 验证 enclave 可信后，由 KMS 下发密钥；
  - 密钥通过 hardware-bound wrapping 存储（如 SGX 的 sealing key 或 SNP 的 VCEK）。
- **运行期间保护：**
  - 内存页标记为 enclave-private（防止泄漏）；
  - 封装 `KeyManager` 模块，只暴露操作接口（如签名/解密），不暴露原始 key；
  - 网络路径走 end-to-end TLS，key 不经 Host；
- **设计原则：** key 不出 enclave、内存不可被 host read、存储不可被物理复制。

### 如何在裁剪版 Linux / LibOS 中开发 mini-container runtime？哪些模块保留？哪些必须重写？

保留：

- `runc` 的容器启动生命周期设计思想；
- 容器镜像管理、OCI runtime format；
- 命名空间机制（可以复用 mount/ns 部分）；

必须重写或替换：

- **cgroup：** 大部分不可用，在 LibOS/TEE 中要靠内部调度器；
- **syscall 代理：** 用 LibOS 内置 syscall shim 处理；
- **网络 & 文件系统：** 需要通过加密通道访问 host 提供的 proxy 服务；
- **volume/mount：** 不支持真实挂载，需要内部构建虚拟层。

本质是实现一个 lightweight runtime，核心是镜像执行、资源隔离、加密 I/O 支持。

## 个人兴趣

### 如果你来做LibOS或Hypervisor开发，哪些方面你最感兴趣？为什么？

我对虚拟内存管理和用户/内核态切换机制很感兴趣。LibOS 中模拟传统 OS 的系统调用、调度、内存管理等机制，而在 TEE 环境下，又需要在可信度、最小TCB之间做权衡。

此外，我也很希望深入设计 secure syscall 接口、信任根的引导过程（如 attestation flow），这部分既有底层的页表管理，也有上层的安全协议，挑战性非常大。

### 如何看待“复合型系统工程”这种挑战？它的价值在哪里？

我非常认可这种“跨层协作”的工程思维。

- 在 TEE、虚拟化、系统安全中，不同层（firmware/kernel/user/app）彼此高度耦合，任何一层设计不当都可能引入攻击面；
- 系统工程师要有全局视角，能从**Boot 到 Runtime、从编译器到安全协议**全面考虑；
- 这种挑战虽然复杂，但**能让系统更可控、更可信、更安全**，而不是碎片化 patchwork。

我非常期待参与这类工程，解决“链路贯通”的问题，同时保持最小 TCB 和高性能，是我追求的理想方向。
