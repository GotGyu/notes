...
- 具体比如：
  ...
  - **async/await**，futures，future API 是一个非常重要的例子，因为早期版本的 future 只满足了零成本抽象的“零成本”部分，但实际上并提供足够好的体验来推动采用。通过添加 pin API 来支持 async/await、跨 await 的引用等等，我们已经开发了一个产品，我认为它将解决用户的问题，并使 Rust 在编写高性能网络服务时更加可行。
...
- **服务端**的职责：长时间运行，维护交易逻辑和状态

  - 市场数据采集和推送：从交易所获取实时行情（如深度数据、K线、tick等）、通过 WebSocket、消息队列或其他方式广播行情数据给客户端
  - 订单管理与撮合处理：接收来自客户端的订单请求、校验并管理订单、反馈下单结果
  - 策略托管与管理：提供策略执行环境、调用策略接口执行交易逻辑、管理策略生命周期和多策略调度

  - 风险控制：实现交易限制、仓位限制、资金管理等风控规则，拦截或拒绝风险订单
  - 通信层：实现ws服务端监听客户端连接，管理连接会话、身份认证、数据序列化

- **客户端**的职责：策略开发者运行的交易接口

  - 行情订阅：通过ws订阅服务端行情数据、实时接收推送的数据并驱动策略运行
  - 策略逻辑执行：根据接收到的市场数据运行策略逻辑、判断买卖点、生成订单请求
  - 订单请求：将生成的订单发送给服务端进行处理、接收服务端返回的下单反馈
  - 状态展示：显示账户、订单、持仓、策略运行状态等信息（如果有可视化界面）
...
1. **资金接入&初始化账户**
   ...
   - 命令行输入金额+账户号码，如 “cargo run -- 1000000 1001”

   - 服务器拉取账户信息（余额、持仓等），初始化策略上下文，给账户1001注资100w

   - 配置好：策略编号 → 使用账户 → 允许证券范围 → 最大资金使用比例（如不超 90w）等。
...
2. **实时行情获取与处理**
   ...
   - 服务器使用ws接口订阅目标股票的实时Ticker, 盘口，k线等数据

   - 所有数据通过 `tokio::spawn` 的任务异步处理，推入 `crossbeam-channel` 给策略模块
...
3. **多策略异步执行**（每个账户一个策略）
   ...
   - 策略使用广播channel订阅行情，每个策略是独立的 `async` 任务

   - 配置的策略会基于 基础指标、K线形态、机器学习模型等，做出交易信号

   - 策略会输出一个“信号”，不是直接下单，而是交由风控模块判断

     ```rust
     if 条件满足 {
         emit_order_signal("BUY", 股票代码, 下单价格, 数量);
     }
     ```

4. **订单管理与风控**

   - 系统检查当前账户是否有足够资金（100w - 已用资金 - 冻结资金）
   - 校验风控规则：如单只股票最大持仓限制、同一时间最多持仓 N 只、若达到止损/止盈线，禁止继续加仓、风控日志打印 + 指标上报

5. **订单成交**

   - 将策略输出转为交易订单（包含股票代码、数量、价格、方向、策略来源、时间戳等），提交到订单通道，异步发送给交易接口
   ...
   - 模拟下单，发下单回执，成功则更新冻结资金、加入订单簿；失败则记录错误，策略重新评估是否重试（带 backoff），由于是模拟下单，本系统内不会失败
...
```swift
            ┌────────────────────────┐
            │      命令行 CLI       │ ← 1000000 1001
            └──────────┬────────────┘
                       ↓
            ┌────────────────────────┐
            │   SQLite 本地账户管理   │ ← init 或更新账户余额
            └──────────┬────────────┘
                       ↓
    ┌─────────────────────────────┐
    │   行情拉取模块（实时）       │ ← 聚宽/东方财富/WebSocket
    └──────┬────────────┬──────────┘
           ↓            ↓
     ┌────────────┐  ┌────────────┐
     │ 策略任务 1 │  │ 策略任务 2 │ ← 每个账户/策略
     └─────┬──────┘  └─────┬──────┘
           ↓               ↓
     ┌─────────────────────────────┐
     │      模拟撮合 & 风控模块     │ ← 本地撮合、更新仓位
     └────────────┬────────────────┘
                  ↓
         ┌───────────────────┐
         │ 持久化到 SQLite     │ ← 写 orders / fills / positions
         └───────────────────┘
```
...
使用 Tokio Runtime 的多线程调度器（`multi_thread` 模式）最大化多核并行处理能力，避免默认单线程调度器带来的瓶颈。
...
- 拆分每个模块为独立 `async` 任务：如行情订阅、策略执行、下单回报处理
- 所有网络 IO（WebSocket/REST）、数据库操作、Redis 调用必须是非阻塞的。

合理使用 select! + 超时机制：

- 防止等待队列长时间阻塞，避免消息淤积
- 对于行情接收、订单响应等关键路径设置超时快速 fail

### 消息处理与流控机制

使用 `crossbeam-channe`l 或 `tokio::mpsc` 实现高性能消息通道，可控制策略与核心引擎之间的通信节奏。支持 backpressure，防止策略“暴走”。

引入速率限制 / 流量控制：

- 对 WebSocket 推送、订单请求进行限流，防止服务器/交易所接口压力过大。如 100ms/次拉取数据、每秒最多 N 次下单。

### 内存 & 数据结构优化

零拷贝数据传输：

- 通过 `Arc<Bytes>` 等方式在模块之间共享行情数据，避免重复解析/复制

使用紧凑数据结构：

- 避免使用 `HashMap<String, String>` 等高开销结构，转而用 `enum`、`bitflags`、`u64` 表示状态和标识

### 策略并发执行优化

热路径最小化（fast path 优化）:

- 策略执行和交易回报处理走最短路径，不进行日志、落库等慢操作。这些慢操作用异步批处理处理（如数据库入库）。

### Redis 缓存 + 降级机制

使用 Redis 作为热点缓存，减轻 DB 压力：行情快照、策略状态、账户状态等频繁访问数据。采用 TTL 控制有效性。

### Rust 特性利用

避免 Mutex / RwLock，改用 Atomic 或无锁结构。
...
- 超时与重试机制：使用 `tokio::time::timeout()` 设置每个任务的最大执行时间；使用 `retry` 算法（如指数退避）重发关键任务（如下单）。

- 错误分类处理：

  - 可恢复错误（如网络中断）：延迟后重试；

  - 不可恢复错误（如策略 panic）：记录日志，通知人工处理，自动重启策略任务。

- 幂等机制：设计订单请求为幂等操作，防止重复下单；使用订单 ID 或请求 ID 做唯一标识。

- 日志与监控：所有异常、超时、失败等都记录详细日志

- 任务重启：可以将策略抽象为 `tokio::task::spawn` 的闭包任务，一旦 panic，可捕获并自动重新调度。
