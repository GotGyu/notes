...
11. 类型转换，i32是否可以传给`i64`形参，写一个泛型函数，同时支持`i64`和`i32`形参
...
17. 用Rust获得安全的同时可能会影响效率，考虑效率优先还是安全优先？

18. 问Rust的时候问xx在C++里对应什么？如指针

19. 程序有哪些section，由什么组成？程序启动的过程？
...
3. 讲一下 Async 的 Future
4. 讲一下 `Send` 和 `Sync`
   - 多线程的时候需要加上，编译器会自己推断一个结构是否 `Send`, 是否 `Sync`
5. Rust中的多线程thread编程和Async异步编程
6. 多线程好处很多,那Nginx为什么使用多进程而不是多线程?
